...
describe 'tty client login' do
  ...
  context 'No success' do
    ...
    it 'fail to login cause, not reachable' do
      allow(double_error_result).to receive(:exit_status).and_return('no host reachable')
      allow(double_error_result).to receive(:out).and_return('no host')
      allow(double_error_result).to receive(:err).and_return('not allowed to login')
